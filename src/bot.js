  ...
  while (true) {
    ...
    try {
      ...
      let file =
        ...
        fs.existsSync(__dirname + "/temp/graphs/graph_" + instanceId + ".png")
          ? [
  ...
  //  playerlist button
    ...
    return gamedig
      ...
      .then((state) => {
        ...
        embed.setTitle(
          "Playerlist (" + state.players.length + "/" + state.maxplayers + "):"
        );
...
async function generateGraph() {
  while (client.token != null) {
    // client.token is not null if it's alive (logged in)
    try {
      // generate graph
      let data = [];
